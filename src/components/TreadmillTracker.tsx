...
import { Play, Square, Timer, Zap, AlertCircle, Activity, X, Camera, MapPin, Lightbulb } from 'lucide-react';
...
// Rhythm detection constants
const RHYTHM_WINDOW_MS = 3000; // analyze last 3 seconds of transitions
const MIN_TRANSITIONS = 5; // minimum motion↔still transitions in window
const MAX_TRANSITIONS = 30; // maximum (too fast = noise)
const MIN_RHYTHM_REGULARITY = 0.4; // how regular transitions must be (0-1)
...
export function TreadmillTracker({ exercise, remainingDailyLP, onComplete, onCancel }: TreadmillTrackerProps) {
  ...
  // Motion tracking state - Rhythm detection
  const transitionTimesRef = useRef<number[]>([]);
  const lastMotionStateRef = useRef<boolean>(false);
  ...
  const processFrame = useCallback(() => {
    ...
    // Record motion↔still transitions for rhythm detection
    if (hasMotion !== lastMotionStateRef.current) {
      lastMotionStateRef.current = hasMotion;
      transitionTimesRef.current.push(ts);
      // Keep only transitions within the analysis window
      const cutoff = ts - RHYTHM_WINDOW_MS;
      transitionTimesRef.current = transitionTimesRef.current.filter(t => t >= cutoff);
    }

    // Analyze rhythm: check if transitions are regular (like running footsteps)
    const recentTransitions = transitionTimesRef.current.filter(t => t >= ts - RHYTHM_WINDOW_MS);
    const transitionCount = recentTransitions.length;
    let isRhythmic = false;

    if (transitionCount >= MIN_TRANSITIONS && transitionCount <= MAX_TRANSITIONS) {
      // Check regularity: calculate intervals between transitions
      const intervals: number[] = [];
      for (let i = 1; i < recentTransitions.length; i++) {
        intervals.push(recentTransitions[i] - recentTransitions[i - 1]);
      }
      if (intervals.length >= 4) {
        const avgInterval = intervals.reduce((a, b) => a + b, 0) / intervals.length;
        // Count how many intervals are within 50% of average
        const regularCount = intervals.filter(iv =>
          iv >= avgInterval * 0.5 && iv <= avgInterval * 1.5
        ).length;
        isRhythmic = (regularCount / intervals.length) >= MIN_RHYTHM_REGULARITY;
      }
    }
            ...
            <div className="lion-card p-6">
              ...
              <div className="space-y-2 p-3 rounded-lg bg-secondary/50">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-medium text-foreground">Rhythm Detection</span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-muted-foreground flex items-center gap-2">
            ...
            <div className="space-y-3">
              <div className="flex items-start gap-3 p-3 rounded-lg bg-secondary/30">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Place your phone on the treadmill console or a phone holder, front camera facing you.</p>
                </div>
              </div>
              <div className="flex items-start gap-3 p-3 rounded-lg bg-secondary/30">
                <Camera className="w-5 h-5 text-primary mt-0.5 shrink-0" />
                <div>
                  <p className="text-sm font-medium text-foreground">Camera Setup</p>
                  <p className="text-xs text-muted-foreground">Face the camera directly. Your face and upper body should be visible, 50–80 cm away.</p>
                </div>
              </div>
              <div className="flex items-start gap-3 p-3 rounded-lg bg-secondary/30">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Keep the phone stable. Run at a steady pace — the camera detects your running rhythm.</p>
                </div>
              </div>
            </div>
